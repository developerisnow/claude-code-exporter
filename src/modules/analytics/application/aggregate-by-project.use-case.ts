...
export class AggregateByProjectUseCase {
  ...
  ): Promise<AggregateByProjectResult> {
  ...
  private async collectPromptsForProject(
    sessions: Session[],
  ): Promise<Prompt[]> {
  ...
  private async saveAggregatedContent(
    ...
    this.logger.log(
      `Aggregated ${prompts.length} prompts for project: ${projectPath}`,
    );
  }
  ...
  private formatPrompts(prompts: Prompt[]): string {
    ...
    for (const prompt of prompts) {
      ...
      // Add prompt
      lines.push(
        `### ${prompt.role.toUpperCase()} - ${prompt.timestamp.toLocaleTimeString()}`,
      );
      lines.push('');
      lines.push('```');
