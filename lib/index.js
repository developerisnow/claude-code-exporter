...
class ClaudePromptExporter {
  ...
  exportAggregate(outputDir) {
      ...
      for (const projectDir of projectDirs) {
        try {
          ...
          filteredSessions.forEach(session => {
            ...
            session.projectName = path.basename(session.projectPath);
    ...
    // Export based on nested option and periodGroup
    ...
    if (this.options.nested && this.options.periodGroup) {
      // Nested by period groups (day/week/month/year)
      ...
      // Nested by projects
      this.log('Using nested export with project grouping');
      return this.exportAggregateNestedByProject(outputDir, allSessions);
    } else {
      // Flat export
  ...
  exportAggregateNestedByProject(outputDir, allSessions) {
    let totalSessionsExported = 0;
    let totalMessagesExported = 0;
    const aggregateTimestamp = this.getTimestamp();
    ...
    Object.entries(projectGroups).forEach(([projectName, sessions]) => {
      // Create project directory with timestamp: aggregated-{timestamp}-{project}
      const projectDir = path.join(outputDir, `aggregated-${aggregateTimestamp}-${projectName}`);
      fs.mkdirSync(projectDir, { recursive: true });

      sessions.forEach(session => {
        const title = this.generateTitle(session.messages, this.options.exportMode);
        const sessionTimestamp = this.getTimestamp();
        
        // Create session directory: {timestamp}-{session-title}
        const sessionDirName = `${sessionTimestamp}-${title}`;
        const sessionDir = path.join(projectDir, sessionDirName);
        fs.mkdirSync(sessionDir, { recursive: true });

        // When "Both" format is selected, export ALL modes
        if (this.options.exportFormat === ExportFormat.BOTH) {
          // Export all three modes for complete output
          const modes = [ExportMode.PROMPTS_ONLY, ExportMode.OUTPUTS_ONLY, ExportMode.FULL_CONVERSATION];
          const originalMode = this.options.exportMode;
          ...
          modes.forEach(mode => {
            this.options.exportMode = mode;
            const modeMessages = this.filterMessagesForMode(session.messages, mode);
            if (modeMessages.length > 0) {
              this.exportSessionMarkdown(sessionDir, session.sessionId, modeMessages, 
                session.stats, title, sessionTimestamp);
              this.exportSessionJSON(sessionDir, session.sessionId, modeMessages, 
                session.stats, title, sessionTimestamp);
            }
          });
          ...
          // Single mode export
          if (this.options.exportFormat === ExportFormat.MARKDOWN || 
              this.options.exportFormat === ExportFormat.JSON) {
            if (this.options.exportFormat === ExportFormat.MARKDOWN) {
              this.exportSessionMarkdown(sessionDir, session.sessionId, session.messages, 
                session.stats, title, sessionTimestamp);
            } else {
              this.exportSessionJSON(sessionDir, session.sessionId, session.messages, 
                session.stats, title, sessionTimestamp);
            }
          }
        }
      ...
      console.log(`✓ ${projectName}: ${sessions.length} sessions, ${sessions.reduce((sum, s) => sum + s.messages.length, 0)} messages`);
    });
  ...
  _detectAllClaudeHomes() {
    // Always return all available Claude homes for aggregate mode
    if (this.options.aggregate) {
      return this._detectClaudeHomes();
    }
    
    // For non-aggregate mode, use bothDirs option
    if (this.options.bothDirs) {
      return this._detectClaudeHomes();
    } else {
